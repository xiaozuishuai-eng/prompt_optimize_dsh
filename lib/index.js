...
export function apply(ctx, config = {}) {
  ...
  // ⚠️ 事故教训：双挂载/热重载竞态下，register 撞 duplicate 会 throw，而落败 fiber 的 disposer
  // 又按 path 键无条件删表——产生"命令活着但 HTTP 全 404"的幽灵态。因此集中定义路由 +
  // 自愈看门狗：每 10s 查表，发现缺失立即接管重注册；本 fiber 持有的 disposer 停用时回收。
  try {
    ...
    if (web) {
      ...
      const ensureWeb = () => {
        try {
          ...
          __poWebErr = String((error && error.message) || error).slice(0, 300)
        }
      }
      ctx.effect(() => {
        ensureWeb()
        const timer = setInterval(ensureWeb, 10000)
        return () => {
          clearInterval(timer)
