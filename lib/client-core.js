...
;(function () {
  ...
  // 消费水位持久化（localStorage）：页面重载/重启后，5 分钟内产生的未消费结果仍可补填
  const SEEN_KEY = 'dshPoSeenSeq'
  let seenSeq = 0
  try { seenSeq = Number(localStorage.getItem(SEEN_KEY)) || 0 } catch { /* ignore */ }
  const markSeen = function (n) {
    lastSeq = n
    seenSeq = n
    try { localStorage.setItem(SEEN_KEY, String(n)) } catch { /* ignore */ }
  }
  ...
  const poll = async () => {
    ...
    if (lastSeq < 0) {
      const freshUnseen = data.seq > 0 && data.seq > seenSeq && data.ts && Date.now() - Number(data.ts) < 5 * 60 * 1000
      if (!freshUnseen) { markSeen(data.seq); return } // 陈旧/已消费：只对齐水位
      lastSeq = seenSeq // 页面关闭期间的最新鲜结果：进入正常消费流补填
    }
    ...
    if (kind === 'clarify') {
      ...
      const prefill = function () {
        ...
        if (cur && cur.value.trim() === '') {
          setComposerText(cur, '/optimize ')
          if (!document.hidden) {
            cur.focus()
            try { cur.setSelectionRange(10, 10) } catch { /* ignore */ }
          }
        }
      }
      prefill()
      toast('❓ 需要补充前提——答案直接打在输入框里发送即可（会自动与原始需求合并）：' + brief, [
        { label: '放入输入框', onClick: prefill },
        { label: '查看', onClick: function () { showFull(full || text) } },
      ], true)
      return
    }
    // fill
    if (data.autoFill !== true) { markSeen(data.seq); return } // 开关关闭：消费但不填入
    if (!text) return
    const ta = findComposer()
    if (!ta) {
